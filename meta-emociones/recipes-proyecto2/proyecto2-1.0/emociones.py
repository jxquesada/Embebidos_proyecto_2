...
def emotionImage(emotion):
	# Emojis
	if emotion == 'Poker': image = cv2.imread('Emojis/poker.jpeg')
	else: #emotion == 'Enojo':
...
dataPath = 'emociones' #Cambia a la ruta donde hayas almacenado Data
...
log.write('Este es el nombre de la pelicula\n')
log.close()
while True:
	...
	for (x,y,w,h) in faces:
		...
		if result[0]!=prevresult[0]:
			...
			log.write(imagePaths[result[0]]+'\t\t'+str(prevtime-inicio)+'\t'+str(time.time()-inicio)+'\n')
			prevtime=time.time()
			prevresult=result
			log.close()
